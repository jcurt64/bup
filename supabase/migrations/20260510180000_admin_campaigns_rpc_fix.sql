-- ════════════════════════════════════════════════════════════════════
-- BUUPP — Fix admin_campaigns_kpis : guard jsonb_array_elements_text
-- ════════════════════════════════════════════════════════════════════
-- Bug : `coalesce(c.targeting->'geo', '[]'::jsonb)` ne rattrape PAS le
-- cas JSON null (`{"geo": null}`) — uniquement SQL NULL. Quand une
-- campagne stocke un targeting incomplet, jsonb_array_elements_text
-- plante avec "cannot extract elements from a scalar" (22023) → 500
-- sur /api/admin/stats/campaigns.
--
-- Fix : on filtre via `jsonb_typeof = 'array'` AVANT d'extraire.
-- ════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- Guard : on n'extrait que si la valeur est bien un array JSON (sinon
  -- jsonb_array_elements_text plante en 22023 sur null/scalaire).
      ...
      select g as geo, count(*) as n
        from public.campaigns c,
             jsonb_array_elements_text(
               case when jsonb_typeof(c.targeting->'geo') = 'array'
                    then c.targeting->'geo'
                    else '[]'::jsonb end
             ) g
  ...
  select coalesce(jsonb_agg(jsonb_build_object('cat', cat, 'n', n) order by n desc), '[]'::jsonb)
    ...
    from (
      select g as cat, count(*) as n
        from public.campaigns c,
             jsonb_array_elements_text(
               case when jsonb_typeof(c.targeting->'categories') = 'array'
                    then c.targeting->'categories'
                    else '[]'::jsonb end
             ) g
        where c.created_at >= p_start and c.created_at < p_end
        group by g order by n desc limit 10
