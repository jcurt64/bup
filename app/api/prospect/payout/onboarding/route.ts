...
export async function POST(request: NextRequest) {
  ...
  // Toute erreur Stripe (ex. profil de plateforme Connect non complété côté
  // compte Stripe live) remonte en JSON lisible au lieu d'un 500 muet.
  try {
    ...
    console.error("[/api/prospect/payout/onboarding] Stripe error", err);
    return NextResponse.json(
      {
        error: "payout_onboarding_unavailable",
        message:
          "L'activation des retraits est momentanément indisponible. Réessayez plus tard ou contactez le support.",
      },
      { status: 503 },
    );
  }
